...
<table>
    ...
    <#-- 一级入参 -->
        <tr>
            ...
            <#-- 三级参数总长度-->
            <#-- 存在二级则向下合并单元格 -->
                <td style="vertical-align: middle" rowspan="${repFirstLevel.childParamSize+1}"><#if repFirstLevel.name??><#if paramType==2>${camelToDashed(repFirstLevel.name)}<#else>${repFirstLevel.name}</#if></#if><#if repFirstLevel.type?index_of("List")!=-1> []<#else> {}</#if></td>
            <#else>
            ...
            <#list repFirstLevel.children as repSecondLevel>
                <tr>
                    ...
                    <#if repSecondLevel.children?? && (repSecondLevel.children?size > 0) >
                        <td style="vertical-align: middle" rowspan="${repSecondLevel.childParamSize+1}"><#if repSecondLevel.name??><#if paramType==2>${camelToDashed(repSecondLevel.name)}<#else>${repSecondLevel.name}</#if></#if><#if repSecondLevel.type?index_of("List")!=-1> []<#else> {}</#if></td>
                    <#else>
                ...
                <#if repSecondLevel.children?? && (repSecondLevel.children?size > 0) >
                    <#list repSecondLevel.children as repThreeLevel>
                        <#if repThreeLevel.children?? && (repThreeLevel.children?size > 0) >
                            <td style="vertical-align: middle" rowspan="${repThreeLevel.childParamSize+1}"><#if repThreeLevel.name??><#if paramType==2>${camelToDashed(repThreeLevel.name)}<#else>${repThreeLevel.name}</#if></#if><#if repThreeLevel.type?index_of("List")!=-1> []<#else> {}</#if></td>
                        <#else>
                            <tr>
                        ...
                        <#if repThreeLevel.children?? && (repThreeLevel.children?size > 0) >
                            ...
                            <#if repFourLevel.children?? && (repFourLevel.children?size > 0) >
                                <td style="vertical-align: middle" rowspan="${repFourLevel.childParamSize+1}"><#if repFourLevel.name??><#if paramType==2>${camelToDashed(repFourLevel.name)}<#else>${repFourLevel.name}</#if></#if><#if repFourLevel.type?index_of("List")!=-1> []<#else> {}</#if></td>
                            <#else>
                                <tr>
