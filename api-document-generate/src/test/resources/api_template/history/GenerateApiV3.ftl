                ...
                <#list repFirstLevel.children as repSecondLevel>
                        ...
                        <#if repSecondLevel.children?? && (repSecondLevel.children?size > 0) >
                            <#list repSecondLevel.children as repThreeLevel>
                                ...
                                <#else>
                                    <tr><#if repThreeLevel_index==0><td style="vertical-align: middle" rowspan="${repSecondLevel.childParamSize-repSecondLevel.hasChildParamSize}"><#if repSecondLevel.name??><#if paramType==2>${camelToDashed(repSecondLevel.name)}<#else>${repSecondLevel.name}</#if></#if><#if repSecondLevel.type?index_of("List")!=-1> []<#else> {}</#if></td></#if>
